...
int
getpinfo(struct proc_stat*process_stat){
	int i = 0;
  	acquire(&ptable.lock);
  ...
  for(;;){
    ...
    // if(argv[2]=="SCHEDULER=MLFQ"){
    	...
    	for(j = 0; j < NPROC; ++j){
          ...
          // cprintf("pid-%d xticks-%d ptable.proc[j].lastexperiencedtick-%d\n",ptable.proc[j].pid,xticks,ptable.proc[j].lastexperiencedtick);
    	...
    	if(c0!=-1){
					for(i=0;i<=c0;i++){
					  ...
					  if(p->ticks[0]!=0 && p->ticks[0]%(clkPerPrio[0]*10)==0){
						  ...
						  // pstat_var.priority[proc->pid] = proc->current_queue;
						  q1[c1] = c->proc;
				...
				// if(c1!=-1)break;
					...
					for(i=0;i<=c2;i++){
					  ...
					  // p->ticks[2]++;
						  ...
						  // pstat_var.priority[p->pid] = p->current_queue;
						  q3[c3] = c->proc;
